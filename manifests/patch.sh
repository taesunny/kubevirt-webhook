...
usage() {
    cat <<EOF
Generate certificate suitable for use with an kubevirt-webhook service.
This script uses k8s' CertificateSigningRequest API to a generate a
certificate signed by k8s CA suitable for use with kubevirt-webhook
...
The following flags are required.
       --service          Service name of kubevirt-webhook.
       --namespace        Namespace where kubevirt-webhook service and secret reside.
...
[ -z ${service} ] && service=kubevirt-webhook
[ -z ${secret} ] && secret=kubevirt-webhook-certs
[ -z ${namespace} ] && namespace=kubevirt-system
...
if ! sed -i "/caBundle/c\    caBundle: $ca" ./kubevirt-webhook.yaml; then
    echo "ERROR: patching kubevirt-webhook.yaml failed"
    exit 1
fi
